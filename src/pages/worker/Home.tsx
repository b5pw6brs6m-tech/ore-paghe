...
import { euro, dataBreve, dataMedia, oreLabel } from '../../lib/format'
    ...
    <>
      ...
      >
        <div className="mt-6 rounded-[26px] bg-white/12 p-5 ring-1 ring-white/20 backdrop-blur">
          <p className="text-[13px] font-medium text-white/70">
            {r.balance >= 0 ? 'Da ricevere adesso' : 'Hai ricevuto in anticipo'}
          </p>
          ...
          <div className="mt-4 grid grid-cols-3 gap-2 border-t border-white/20 pt-4 text-center">
            <MiniStat v={String(r.days)} t={r.days === 1 ? 'giorno' : 'giorni'} />
            <MiniStat v={oreLabel(r.totalHours)} t="ore totali" />
            <MiniStat v={`${euro(worker.hourly_rate)}`} t="all’ora" />
          </div>
        </div>
      </Header>
        ...
        <div className="px-5 pt-5">
          <div className="animate-rise flex items-center gap-3 rounded-3xl bg-emerald-50 px-4 py-4 ring-1 ring-emerald-200">
            ...
            <div className="min-w-0">
              <p className="text-[14px] font-bold text-emerald-900">Registrato!</p>
              <p className="truncate text-[13px] capitalize text-emerald-700">
                {dataMedia(conferma.data)} · {oreLabel(conferma.ore)} · {euro(conferma.guadagno)}
              </p>
            </div>
          </div>
        </div>
      ...
      <Sezione titolo="Il tuo conto">
        <Card className="divide-y divide-ink-100">
          <RigaConto etichetta="Hai guadagnato" valore={euro(r.totalEarned)} colore="text-ink-900" />
          <RigaConto etichetta="Ti ho già pagato" valore={`− ${euro(r.totalPaid)}`} colore="text-emerald-600" />
          <div className="flex items-center justify-between px-5 py-4">
            <span className="text-[15px] font-bold text-ink-900">
              {r.balance >= 0 ? 'Ti devo ancora' : 'Hai avuto in più'}
            </span>
      ...
      <Sezione
        ...
        azione={ultimi.length > 0 && (
          <button onClick={() => nav('/ore')} className="flex items-center gap-1 text-[13px] font-semibold text-brand-600">
            Vedi tutte <IconRight className="h-3.5 w-3.5" />
          </button>
        )}
      >
        {ultimi.length === 0 && payments.length === 0 ? (
          <Vuoto
            ...
            titolo="Ancora nessuna giornata"
            testo="Appena registri le ore le trovi qui, con il guadagno di ogni giorno."
            azione={<Button variant="soft" onClick={() => nav('/registra')}>Registra la prima giornata</Button>}
          />
        ) : (
  ...
  return (
    <Card className="flex items-center gap-4 px-4 py-3.5">
      ...
      <div className="min-w-0 flex-1">
        <p className="text-[15px] font-bold capitalize text-ink-900">{dataBreve(e.work_date)}</p>
        <p className="truncate text-[13px] text-ink-500">
          {e.start_time && e.end_time
            ? `${e.start_time} – ${e.end_time}${e.break_minutes ? ` · pausa ${e.break_minutes}m` : ''}`
            : 'ore inserite a mano'}
          {e.note ? ` · ${e.note}` : ''}
        </p>
      </div>
